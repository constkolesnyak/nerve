...
# --- Setup agent prompt for worker mode ---

_SETUP_AGENT_PROMPT = """\
You are a setup agent configuring a Nerve worker instance. The user will describe \
what they want the worker to do. Your job is to generate a complete configuration.

Generate a JSON object with these keys:

1. **task_md** (string): A structured TASK.md in markdown with these sections:
...
2. **cron_jobs** (array of objects): Each job needs:
   - id: short kebab-case identifier
   - schedule: cron expression (e.g. "*/15 * * * *")
   - description: human-readable description
   - prompt: detailed instructions for the agent executing this cron
   - session_mode: "persistent" (for monitoring that needs context) or "isolated" (for one-off analysis)
   - context_rotate_hours: optional, hours before rotating session context (default: 24 for persistent)

   The cron prompts should reference Nerve tools: task_create, plan_propose, notify, \
ask_user, memory_recall, memorize. The worker has bash, file read/write, web fetch, \
and web search available.

3. **skills** (array of objects): Each skill needs:
   - id: short kebab-case identifier
   - name: human-readable name
   - description: what the skill does and when to use it (include trigger phrases)
   - content: full SKILL.md content with YAML frontmatter (--- delimited block with \
name and description) and markdown body with step-by-step instructions
...
class SetupChoices:
    ...
    worker_config: dict[str, Any] | None = None  # Generated by setup agent
...
class SetupWizard:
    ...
    def _step_worker_setup(self) -> None:
        """Collect task description, run setup agent to generate configs."""
        click.clear()
        ...
        # Check if we have API access for the setup agent
        has_api = bool(self.choices.anthropic_api_key) or self.choices.use_proxy
        if not has_api:
            click.secho(
                "  No API key or proxy configured — skipping setup agent.\n"
                "  TASK.md will contain your raw description. You can\n"
                "  configure crons and skills manually later.",
                fg="yellow",
            )
            click.echo()
            return

        click.secho("  The setup agent will analyze your task and generate:", dim=True)
        click.secho("    • A structured TASK.md", dim=True)
        click.secho("    • Custom cron jobs for monitoring", dim=True)
        click.secho("    • Task-specific skills", dim=True)
        click.echo()

        if not click.confirm("  Run setup agent?", default=True):
            click.secho("  → Skipping — using raw task description.", dim=True)
            click.echo()
            return

        click.echo()
        click.secho("  ⏳ Setup agent is analyzing your task...", fg="cyan")
        click.echo()
        ...
        try:
            result = asyncio.get_event_loop().run_until_complete(
                self._run_setup_agent(self.choices.task_description)
            )
        except Exception as e:
            click.secho(f"  ✗ Setup agent failed: {e}", fg="red")
            click.secho(
                "  Falling back to raw task description. You can\n"
                "  configure crons and skills manually later.",
                dim=True,
            )
            click.echo()
        ...
        self._preview_worker_config(result)

        if click.confirm("  Apply this configuration?", default=True):
            self.choices.worker_config = result
            click.echo()
            click.secho("  ✓ Worker configuration accepted", fg="green")
        else:
            click.secho("  → Using raw task description instead.", dim=True)
            self.choices.worker_config = None
        click.echo()

    async def _run_setup_agent(self, task_description: str) -> dict[str, Any]:
        """Run a lightweight Claude session to generate worker configs."""
        import anthropic
        ...
        if self.choices.use_proxy:
            ...
            api_key = proxy_cfg.api_key  # default local proxy key
            base_url = f"http://{proxy_cfg.host}:{proxy_cfg.port}/v1/"

            # Start proxy temporarily
            tmp_config = NerveConfig(proxy=proxy_cfg)
            from nerve.proxy.service import ProxyService
            proxy = ProxyService(tmp_config)
            await proxy.start()
        else:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": (
                            f"Set up a worker for this task:\n\n{task_description}"
                        ),
                    }
                ],
            )
    ...
    def _parse_setup_response(self, text: str) -> dict[str, Any]:
        """Parse the setup agent's JSON output robustly."""
        ...
        def _try_parse(s: str) -> dict | None:
            """Try parsing JSON, tolerant of control characters."""
            try:
        ...
        if data is None:
            ...
            if data is None:
                raise ValueError(
                    f"Setup agent returned invalid JSON.\n\n"
                    f"Raw output:\n{text[:500]}"
                )
        ...
        # Validate and normalize cron jobs
        for job in data.get("cron_jobs", []):
            if not isinstance(job, dict):
                continue
            if not job.get("id") or not job.get("schedule") or not job.get("prompt"):
            ...
            result["cron_jobs"].append({
                ...
                "context_rotate_hours": job.get("context_rotate_hours"),
            })
        ...
        for skill in data.get("skills", []):
            if not isinstance(skill, dict):
                continue
            skill_id = skill.get("id") or skill.get("name", "").lower().replace(" ", "-")
            content = skill.get("content", "")
            if not skill_id or not content:
                continue  # Skip incomplete skills
    ...
    def _preview_worker_config(self, config: dict[str, Any]) -> None:
        """Display the generated worker configuration for review."""
        click.secho("  ┌──────────────────────────────────────────────────┐", dim=True)
        click.secho("  │   Worker Configuration (generated by Claude)     │", bold=True)
        click.secho("  ├──────────────────────────────────────────────────┤", dim=True)

        # TASK.md preview
        task_md = config.get("task_md", "")
        if task_md:
            click.secho("  │                                                  │", dim=True)
            click.secho("  │  TASK.md:                                        │", bold=True)
            # Show first few meaningful lines
            for line in task_md.split("\n")[:8]:
                line = line.strip()
                if line:
                    display = line[:48]
                    click.secho(f"  │    {display:<46}│", dim=True)

        # Cron jobs
        crons = config.get("cron_jobs", [])
        if crons:
            click.secho("  │                                                  │", dim=True)
            click.secho("  │  Cron Jobs:                                      │", bold=True)
            for job in crons:
                name = job["id"][:20]
                schedule = job["schedule"][:18]
                click.secho(f"  │    ✓ {name:<20} ({schedule})       │"[:55] + "│", dim=True)
                desc = job.get("description", "")[:44]
                if desc:
                    click.secho(f"  │      {desc:<44}│", dim=True)

        # Skills
        skills = config.get("skills", [])
        if skills:
            click.secho("  │                                                  │", dim=True)
            click.secho("  │  Skills:                                         │", bold=True)
            for skill in skills:
                name = skill.get("name", skill["id"])[:44]
                click.secho(f"  │    ✓ {name:<44}│", dim=True)
                desc = skill.get("description", "")[:44]
                if desc:
                    click.secho(f"  │      {desc:<44}│", dim=True)
    ...
    def _step_review(self) -> None:
        ...
        if self.choices.mode == "personal":
            ...
            if self.choices.worker_config:
                n_crons = len(self.choices.worker_config.get("cron_jobs", []))
                n_skills = len(self.choices.worker_config.get("skills", []))
                click.secho(f"  │  Setup:      {n_crons} crons, {n_skills} skills (generated){'':<7}│"[:51] + "│")
            else:
                click.secho("  │  Setup:      manual (no setup agent)          │")
    ...
    def _apply(self) -> None:
        ...
        if self.choices.mode == "worker" and self.choices.task_description:
            ...
            wc = self.choices.worker_config
            if wc and wc.get("task_md"):
                # Use the structured TASK.md from the setup agent
                task_md.write_text(wc["task_md"], encoding="utf-8")
            else:
        ...
        # 4. Write generated skills for worker mode
        if self.choices.mode == "worker" and self.choices.worker_config:
            skills = self.choices.worker_config.get("skills", [])
            if skills:
                click.echo("  Writing generated skills...", nl=False)
                ...
                click.secho(f" ✓ ({len(skills)} skills)", fg="green")
    ...
    def _write_cron_jobs(self) -> None:
        ...
        if self.choices.mode == "personal":
            ...
            for cron in PRODUCTIVITY_CRONS:
                ...
                jobs.append(job)
        elif self.choices.mode == "worker" and self.choices.worker_config:
            # Worker-generated crons from setup agent
            for cron in self.choices.worker_config.get("cron_jobs", []):
                job = {
                    "id": cron["id"],
                    "schedule": cron["schedule"],
                    "prompt": cron["prompt"],
...
def run_non_interactive(config_dir: Path) -> SetupChoices:
    ...
    # Worker task description + setup agent
    if choices.mode == "worker":
        choices.task_description = os.environ.get("NERVE_TASK", "")

        # Run setup agent non-interactively if task description is provided
        if choices.task_description:
            has_api = bool(choices.anthropic_api_key) or choices.use_proxy
            if has_api:
                import asyncio

                click.echo("Running setup agent...")
                tmp_wizard = SetupWizard(config_dir, inside_docker=is_docker)
                tmp_wizard.choices = choices
                try:
                    result = asyncio.get_event_loop().run_until_complete(
                        tmp_wizard._run_setup_agent(choices.task_description)
                    )
                    choices.worker_config = result
                    click.echo(
                        f"  Generated: {len(result.get('cron_jobs', []))} crons, "
                        f"{len(result.get('skills', []))} skills"
                    )
                except Exception as e:
                    click.echo(f"  Setup agent failed: {e} — using raw task description")
